...
AgentFence 通过隔离环境、结构化审计轨迹和可重复工件来回答这些问题。

## AgentFence 做什么

AgentFence 在隔离的 Docker 容器中使用 git worktree 隔离运行 coding agent，
...
**当前状态：** Tier 0 stabilization in progress。Claude Code Tier 2 hook
adapter 模块已存在，但 v0.1 的 CLI `run` 路径当前使用 command adapter。
...
- 每次运行即使失败也生成可审计工件（manifest、events、logs、patch、report）
- 每次运行前后验证原始工作区指纹
...
AgentFence 是一个**安全 Harness**，不是安全沙箱。当你显式启用网络（`--network bridge`）时，agent 可以发起出站连接。
...
运行 `agentfence run` 后生成的 HTML 报告包含 8 个章节：
...
AgentFence 是面向 coding agent 的安全执行 Harness。它**不**执行代码生成、LLM 推理或自主修复。它**不**保证"完全安全"或"绝对隔离"。它提供的是一个结构化、可审计、可复现的执行环境，使得 agent 的行为可以被审查、重放和对比。
