...
| 等级 | 名称 | 能力 |
|------|------|------|
| 0 | Process Wrapper | 容器隔离、工件记录、patch 导出、资源监控、best-effort HTML 报告 |
...
- 容器默认无网络、未启用 Docker privileged mode，并设置 CPU/内存/PID
  限制；所有 Linux capability 默认 drop，仅保留 `DAC_OVERRIDE`，用于让容器内
  root 写入 bind-mounted workspace
- 环境变量通过显式 allowlist 注入——绝不传递整个主机环境
- 每次沙箱执行尝试即使失败也生成核心可审计工件（manifest、events、logs、patch）；
  JSON/HTML 报告为 best-effort
- 每次沙箱执行前后验证原始工作区指纹
