...
PatchGuard 通过隔离环境、结构化审计轨迹和可重复工件来回答这些问题。

## PatchGuard 做什么

PatchGuard 在隔离的 Docker 容器中使用 git worktree 隔离运行 coding agent，
...
PatchGuard 是一个**安全 Harness**，不是安全沙箱。当你显式启用网络（`--network bridge`）时，agent 可以发起出站连接。
...
运行 `patchguard run` 后生成的 HTML 报告包含 8 个章节：
...
PatchGuard 是面向 coding agent 的安全执行 Harness。它**不**执行代码生成、LLM 推理或自主修复。它**不**保证"完全安全"或"绝对隔离"。它提供的是一个结构化、可审计、可复现的执行环境，使得 agent 的行为可以被审查、重放和对比。
