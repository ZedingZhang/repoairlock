"""AgentFence CLI — safety-oriented execution harness for coding agents."""
...
def main(
    ...
    """AgentFence — safety-oriented execution harness for coding agents.
...
@app.command()
def doctor() -> None:
    """Check that the local environment is ready for AgentFence."""
    ...
    # --- Print results ---
    print("AgentFence Doctor — environment check")
    print("=" * 50)
    ...
    if failures == 0:
        print("\nAll checks passed — AgentFence is ready.")
    else:
        print(f"\n{failures} check(s) failed. Fix issues before running agentfence.")
    ...
    try:
        r = subprocess.run(
            [
                ...
                "alpine:latest", "echo", "agentfence-doctor-ok",
            ],
            capture_output=True, text=True, check=False, timeout=60,
        )
        if r.returncode == 0 and "agentfence-doctor-ok" in r.stdout:
...
def _check_worktree(git_available: bool) -> tuple[bool, str]:
    ...
    try:
        repo = Path(tempfile.mkdtemp(prefix="agentfence-dr-"))
        subprocess.run(
        ...
        tmp = Path(tempfile.gettempdir()) / "agentfence-doctor-wt-test"
        subprocess.run(
...
def run(
    ...
    Example:
        agentfence run --repo ./my-repo --image python:3.12 -- python agent.py
    """
    if agent_command is None:
        print("Error: No agent command provided. Use -- before the agent command.")
        print("Example: agentfence run --repo ./repo --image alpine -- echo hello")
    ...
    try:
        from agentfence.core.orchestrator import RunOrchestrator
        orchestrator = RunOrchestrator()
        result = orchestrator.execute(config)
    except AgentFenceError as e:
        print(f"Error: {e}")
...
def inspect(
    ...
    # Replay hint
    print(f"\n  Replay: agentfence replay {run_id}")
