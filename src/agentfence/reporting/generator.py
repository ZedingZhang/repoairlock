...
from agentfence.analysis.diff_metrics import compute_diff_metrics
from agentfence.analysis.quality_score import compute_quality_findings
from agentfence.analysis.trajectory_metrics import compute_trajectory_metrics
...
class ReportGenerator:
    ...
    def generate(self) -> dict[str, object]:
        ...
        report: dict[str, object] = {
            ...
            "replay_instructions": {
                "command": f"agentfence replay {self._run_id}",
                "note": (
...
def _tier_description(tier: str) -> str:
    descriptions = {
        "tier_0_process_wrapper": (
            "Process Wrapper: AgentFence observes and records process-level "
        ...
        "tier_1_structured_events": (
            "Structured Events: AgentFence imports agent tool-call traces for "
            "per-step observability."
        ),
        "tier_2_enforcement": (
            "Enforcement: AgentFence intercepts and can block individual tool "
...
<html lang="en">
<head><meta charset="utf-8"><title>AgentFence Report — {run_id}</title>
...
<p><em>Generated by AgentFence v0.1.0</em></p>
</body></html>"""
