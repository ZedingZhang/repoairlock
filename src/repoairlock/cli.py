"""RepoAirlock CLI — safety-oriented execution harness for coding agents."""
...
from repoairlock.constants import APP_NAME, DEFAULT_RUNS_DIR
from repoairlock.core.orchestrator import RunConfig
from repoairlock.exceptions import RepoAirlockError
...
def main(
    ...
    """RepoAirlock — safety-oriented execution harness for coding agents.
...
@app.command()
def doctor() -> None:
    """Check that the local environment is ready for RepoAirlock."""
    ...
    # --- Print results ---
    print("RepoAirlock Doctor — environment check")
    print("=" * 50)
    ...
    if failures == 0:
        print("\nAll checks passed — RepoAirlock is ready.")
    else:
        print(f"\n{failures} check(s) failed. Fix issues before running repoairlock.")
    ...
    try:
        r = subprocess.run(
            [
                ...
                "alpine:latest", "echo", "repoairlock-doctor-ok",
            ],
            capture_output=True, text=True, check=False, timeout=60,
        )
        if r.returncode == 0 and "repoairlock-doctor-ok" in r.stdout:
...
def _check_worktree(git_available: bool) -> tuple[bool, str]:
    ...
    try:
        repo = Path(tempfile.mkdtemp(prefix="repoairlock-dr-"))
        subprocess.run(
        ...
        tmp = Path(tempfile.gettempdir()) / "repoairlock-doctor-wt-test"
        subprocess.run(
...
def run(
    ...
    """Run a coding agent inside RepoAirlock's isolated sandbox.

    All arguments after -- are passed to the agent as its command.

    Example:
        repoairlock run --repo ./my-repo --image python:3.12 -- python agent.py
    """
    if agent_command is None:
        print("Error: No agent command provided. Use -- before the agent command.")
        print("Example: repoairlock run --repo ./repo --image alpine -- echo hello")
    ...
    try:
        from repoairlock.core.orchestrator import RunOrchestrator
        orchestrator = RunOrchestrator()
        result = orchestrator.execute(config)
    except RepoAirlockError as e:
        print(f"Error: {e}")
    ...
    print(f"\nInspect: repoairlock inspect {result.run_id}")
...
def inspect(
    ...
    # Replay hint
    print(f"\n  Replay: repoairlock replay {run_id}")
