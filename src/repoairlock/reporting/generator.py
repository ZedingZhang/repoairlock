...
from repoairlock.analysis.diff_metrics import compute_diff_metrics
from repoairlock.analysis.quality_score import compute_quality_findings
from repoairlock.analysis.trajectory_metrics import compute_trajectory_metrics
...
def _tier_description(tier: str) -> str:
    descriptions = {
        "tier_0_process_wrapper": (
            "Process Wrapper: RepoAirlock observes and records process-level "
        ...
        "tier_1_structured_events": (
            "Structured Events: RepoAirlock imports agent tool-call traces for "
            "per-step observability."
        ),
        "tier_2_enforcement": (
            "Enforcement: RepoAirlock intercepts and can block individual tool "
...
<html lang="en">
<head><meta charset="utf-8"><title>RepoAirlock Report — {run_id}</title>
<style>{css}</style></head>
<body>
<h1>RepoAirlock Run Report</h1>
<div class="meta">
...
<p><em>Generated by RepoAirlock v{VERSION}</em></p>
</body></html>"""
