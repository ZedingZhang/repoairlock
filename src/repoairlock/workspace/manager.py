...
from repoairlock.exceptions import CleanupError, WorkspaceError
from repoairlock.workspace.fingerprint import (
...
class WorkspaceManager:
    ...
    def prepare(
        ...
        # Create worktree
        worktree_path = self._tmp_root / f"repoairlock-wt-{run_id}"
    ...
    def cleanup(
        ...
        # Verify worktree metadata is fully cleaned
        stale = self._git.worktree_list_porcelain(toplevel)
        worktree_name = worktree.name
        for entry in stale:
            wt_path = entry.get("worktree", "")
            if worktree_name in wt_path or str(worktree) in wt_path:
                _warn_manual_cleanup(worktree, toplevel)
                raise CleanupError(
                    f"Worktree still present in git metadata after cleanup: {worktree}. "
                    f"Manually run: git -C {toplevel} worktree remove --force {worktree}"
                )
