...
RepoAirlock to intercept and potentially block Claude Code tool calls
...
from repoairlock.adapters.base import AdapterLaunchSpec, AgentAdapter
...
class ClaudeCodeAdapter(AgentAdapter):
    ...
    through a RepoAirlock hook handler for policy enforcement and event
    ...
    def build_launch_spec(
        ...
        *,
        ...
        hooks_dir = Path(tempfile.mkdtemp(prefix=f"repoairlock-hooks-{run_id}-"))
...
"""RepoAirlock PreToolUse / PostToolUse hook handler.

Reads hook input from stdin (JSON), checks tools against policy engine,
records events, and returns allow/deny decisions.

Environment variables expected:
  REPOAIRLOCK_EVENTS_PATH — path to the run's events.jsonl
  REPOAIRLOCK_RUN_ID — the run ID for event recording
"""
...
def main():
    ...
    run_id = os.environ.get("REPOAIRLOCK_RUN_ID", "unknown")
    events_path = os.environ.get("REPOAIRLOCK_EVENTS_PATH", "")
